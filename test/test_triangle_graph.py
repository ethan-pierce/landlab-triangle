...
@pytest.fixture
def square_graph():
    return DualTriangleGraph([0, 10, 10, 0], [0, 0, 11, 11], triangle_options="pqa1Djevz")


@pytest.mark.parametrize("point", ("corner", "node"))
def test_all_points_in_box(square_graph, point):
    x, y = (
        getattr(square_graph, f"x_of_{point}"),
        getattr(square_graph, f"y_of_{point}"),
    )
...
def test_graph_init():
    graph = DualTriangleGraph([0, 10, 10, 0], [0, 0, 11, 11], triangle_options="pqa1Djevz")
    assert graph.number_of_corners == graph.number_of_patches


def test_graph_init_zero_length_edges():
    with pytest.raises(RuntimeError):
        DualTriangleGraph([0, 10, 10, 0], [0, 0, 10, 10], triangle_options="pqa1Djevz")


def test_raise_error_if_no_interior_nodes(geojson_concave_polygon):
    """If no cells are generated, raise a ValueError."""
    with pytest.raises(ValueError):
        DualTriangleGraph.from_vector_file(geojson_concave_polygon, triangle_options="pqa100Djevz")


def test_circular_polygon(geojson_circular_polygon):
    graph = DualTriangleGraph.from_vector_file(
        geojson_circular_polygon,
        triangle_options="pqDjevz",
    )
...
def test_generate_graph_from_geojson(geojson_concave_polygon):
    ...
    graph = DualTriangleGraph.from_vector_file(
        geojson_concave_polygon, triangle_options="pqa10Djevz"
    )
    ...
    assert_array_equal(graph.nodes_at_link[:3], [[18, 8], [20, 18], [20, 8]])
...
def test_multiple_interior_rings(geojson_interior_rings):
    graph = DualTriangleGraph.from_vector_file(
        geojson_interior_rings, triangle_options="pq10a10Djevz"
    )
...
def test_invalid_polygon_error():
    ...
    with pytest.raises(ValueError):
        DualTriangleGraph(xs, ys, triangle_options="pqa0.1Devjz")
